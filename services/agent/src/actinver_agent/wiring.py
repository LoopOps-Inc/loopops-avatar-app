...
async def build_dependencies(settings: Settings) -> Dependencies:
    ...
    if settings.core_provider == "synthetic":
    ...
    elif settings.core_provider == "sql":
        ...
        from actinver_agent.persistence.db import create_engine

        dataset_engine = create_engine(settings.database_url)
        closers.append(dataset_engine.dispose)
    ...
    if settings.voice.provider == "google":  # pragma: no cover - needs GCP credentials
        ...
        # Local-only speech bridge: real synthesis and utterance-level
        # transcription with the AI Studio key (ADR-0003).
        from actinver_agent.adapters.gemini import (
            GeminiClientFactory,
            GeminiSpeechToText,
            GeminiTextToSpeech,
        ...
        stt = GeminiSpeechToText(voice_factory, settings)
